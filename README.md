...
Markdownとスライドの**どちらからでも**編集できます。スライド側はパワーポイントのように、要素の移動・リサイズ・図形の挿入まで行えます。

**文字の編集(双方向)**

- ブロック(見出し・段落・箇条書き・引用・表のセル・コード)をダブルクリック、または選択中にもう一度クリックすると、その場で書き換えられます
...
**自由配置・図形(Markdownは変えない)**

- ブロックをクリックして選択 → ドラッグで移動、ハンドルでリサイズ。矢印キーで微調整(Shiftで大きく)
- 挿入ツールバーから、任意の位置に**テキストボックス**や**図形**(四角形・円・三角形・直線・矢印)を追加できます
- **位置・サイズ・図形は Markdown に書き込まれません**(レイアウト情報として別に保存)。動かしただけ・図形を足しただけなら md は変わりません。テキストボックスに打った**文字内容だけ**が md に入ります
- 選択して Delete で削除。書き出し(PDF / PPTX)にも配置・図形がそのまま反映されます
- ツールバーの「スライドを直接編集」で切り替え。発表(全画面)中は自動でオフになります
...
- `src/overlay.ts` — 自由配置・図形のレイヤ(位置/サイズ/図形をMarkdown外に保持)
...
- **100テーマを体系生成** — 色・明度・書体を規則的に組み、どれも統一感のある仕上がりにする。本文・リンク・小見出しの配色はWCAG AA(コントラスト比4.5以上)を全テーマで満たすよう明度を自動調整し、テストで担保する
