...
- 挿入ツールバーから、任意の位置に**テキストボックス**・**図形**(四角形・円・三角形・直線・矢印)・**画像**を追加できます
- 画像はツールバーの画像ボタン・スライドへの**ドラッグ&ドロップ**・**貼り付け**(Cmd/Ctrl+V)で取り込め、**Markdownに挿入**するか**スライドに自由配置**するかを選べます。自由配置の画像は角ハンドルで縦横比を保ってリサイズできます(Shiftで自由変形)
- **位置・サイズ・図形・自由配置画像は Markdown に書き込まれません**(レイアウト情報として別に保存)。動かしただけ・図形や画像を足しただけなら md は変わりません。テキストボックスに打った**文字内容だけ**が md に入ります
- 選択して Delete で削除。書き出し(PDF / PPTX / 画像)にも配置・図形・画像がそのまま反映されます
