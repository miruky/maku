...
describe('parseDeck', () => {
  ...
  it('manual: 無印ブロックは常時表示(step0)、番号付きだけが順に進む', () => {
    const s = parseDeck('<!-- reveal: manual -->\n\nA\n\n<!-- step: 1 -->\nB\n\n<!-- step: 2 -->\nC').slides[0]!;
    expect(s.reveal).toBe('manual');
    expect(s.steps?.map((x) => x.step)).toEqual([0, 1, 2]);
  });

  it('manual: マーカー皆無なら全ブロック常時表示(step0)', () => {
    const s = parseDeck('<!-- reveal: manual -->\n\nA\n\nB\n\nC').slides[0]!;
    expect(s.steps?.every((x) => x.step === 0)).toBe(true);
  });

  it('manual: group は直前の番号と同時に出る', () => {
    const s = parseDeck('<!-- reveal: manual -->\n\n<!-- step: 1 -->\nA\n\n<!-- group -->\nB\n\nC').slides[0]!;
    expect(s.steps?.map((x) => x.step)).toEqual([1, 1, 0]);
  });

  it('manual: 先行する番号が無い group は常時表示(0)。番号付きが先にあればその番号', () => {
    // 回帰防止: 孤立 group が幻のステップ1にならない。
    expect(parseDeck('<!-- reveal: manual -->\n\nA\n\n<!-- group -->\nB').slides[0]!.steps?.map((x) => x.step)).toEqual([0, 0]);
    expect(parseDeck('<!-- reveal: manual -->\n\nA\n\n<!-- group -->\nB\n\n<!-- step: 2 -->\nC').slides[0]!.steps?.map((x) => x.step)).toEqual([0, 0, 1]);
  });

  it('manual: key 単独は常時表示(step0)+キー扱い。番号で出すには step:N と併用', () => {
    const s = parseDeck('<!-- reveal: manual -->\n\n<!-- key -->\nA\n\nB').slides[0]!;
    expect(s.steps?.map((x) => x.step)).toEqual([0, 0]);
    expect(s.steps?.[0]?.key).toBe(true);
    const staged = parseDeck('<!-- reveal: manual -->\n\n<!-- key -->\n<!-- step: 1 -->\nA\n\nB').slides[0]!;
    expect(staged.steps?.[0]?.step).toBe(1);
  });

  it('manual 追加後も sequential/key-first は従来どおり(回帰防止)', () => {
    expect(parseDeck('<!-- incremental -->\n# A\n\nB\n\nC').slides[0]!.steps?.map((x) => x.step)).toEqual([1, 2, 3]);
    const kf = parseDeck('<!-- reveal: key-first -->\n# 見出し\n\n本文1\n\n本文2').slides[0]!;
    expect(kf.steps?.[0]).toEqual({ step: 1, key: true });
  });
...
describe('setBlockMarker(ブロック単位の段階表示マーカー)', () => {
  const src = '# 見出し\n\n本文1\n\n本文2';
  const at = (needle: string): number => src.indexOf(needle);

  it('指定ブロックの直前にマーカー行を挿入する', () => {
    expect(setBlockMarker(src, at('本文2'), 'key')).toBe('# 見出し\n\n本文1\n\n<!-- key -->\n本文2');
    expect(setBlockMarker(src, at('# 見出し'), 'pin')).toBe('<!-- pin -->\n# 見出し\n\n本文1\n\n本文2');
  });

  it('既存マーカーは置き換える(重複しない)', () => {
    const withKey = '# 見出し\n\n<!-- key -->\n本文1';
    expect(setBlockMarker(withKey, withKey.indexOf('本文1'), 'group')).toBe('# 見出し\n\n<!-- group -->\n本文1');
  });

  it('auto は既存マーカーを取り除くだけ(新規挿入しない)', () => {
    const withPin = '<!-- pin -->\n# 見出し\n\n本文1';
    expect(setBlockMarker(withPin, withPin.indexOf('# 見出し'), 'auto')).toBe('# 見出し\n\n本文1');
  });

  it('parseDeck と往復して step/role が反映される', () => {
    const out = setBlockMarker('<!-- reveal: key-first -->\n\n前\n\n本命\n\n後', '<!-- reveal: key-first -->\n\n前\n\n本命\n\n後'.indexOf('本命'), 'key');
    const s = parseDeck(out).slides[0]!;
    const keyIdx = s.steps?.findIndex((x) => x.key);
    expect(keyIdx).toBe(1); // 「本命」がキー
  });

  it('空行を挟んだ既存マーカーも置換・除去できる(重複/解除不能を防ぐ)', () => {
    const withGap = '<!-- reveal: sequential -->\n\n<!-- key -->\n\n本文';
    const at = withGap.indexOf('本文');
    // auto: 既存 key が確実に消える
    const cleared = setBlockMarker(withGap, at, 'auto');
    expect(cleared).not.toContain('<!-- key -->');
    // group: key を置き換える(両方残さない)
    const grouped = setBlockMarker(withGap, at, 'group');
    expect((grouped.match(/<!--/g) ?? []).length).toBe(2); // reveal と group の2つだけ
    expect(grouped).toContain('<!-- group -->');
    expect(grouped).not.toContain('<!-- key -->');
  });

  it('step:N で順番を割り当て、parseDeck に反映される', () => {
    const base = '<!-- reveal: sequential -->\n\nA\n\nB\n\nC';
    let out = setBlockMarker(base, base.indexOf('C'), 'step:1');
    out = setBlockMarker(out, out.indexOf('A'), 'step:2');
    const s = parseDeck(out).slides[0]!;
    const steps = s.steps?.map((x) => x.step) ?? [];
    // C=1, A=2 を指定。B は自動。連番化されるが C の方が A より先(小さい)になる。
    const ai = s.steps ? 0 : -1;
    expect(ai).toBe(0);
